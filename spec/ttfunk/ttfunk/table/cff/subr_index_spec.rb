...
RSpec.describe TTFunk::Table::Cff::SubrIndex do
  ...
  it 'parses subroutines correctly' do
    expect(font_index[1].private_dict.subr_index[3].bytes).to(
      eq([39, 10, 108, 116, 37, 10, 11])
    )

    expect(font_index[15].private_dict.subr_index[6].bytes).to(
      eq([174, 3, 34, 10, 11])
    )

    expect(font_index[18].private_dict.subr_index[50].bytes).to(
      eq([179, 172, 173, 179, 180, 172, 105, 99, 11])
    )
  end
